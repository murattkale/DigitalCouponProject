...
public static class ServerSideProcessor
{
    ...
    public static IQueryable<T> ToSorting<T>(this IQueryable<T> table, DTParameters<T> Param)
    {
        ...
        if (Param.SortOrder != null && Param.SortOrder != "Image" && Param.SortOrder != "Parent" && Param.SortOrder != "EXPIREDDATE" && Param.SortOrder != "LASTEXPIREDDATE"
            && Param.SortOrder != "sitePrice" && Param.SortOrder != "Detail"
            && Properties.Any(p => p.Name == Param.SortOrder) && cl != null && cl.Count > 0 && Properties.Any(p => cl.Contains(p.Name)))
        {
